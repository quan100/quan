...
class MavenPublishingConventions {
    ...
    private void customizePom(MavenPom pom, Project project) {
        pom.getUrl().set("https://gitee.com/quan100/quan");
    ...
    private void customizeOrganization(MavenPomOrganization organization) {
        organization.getName().set("Quan App");
        organization.getUrl().set("https://gitee.com/quan100/quan");
    }

    private void customizeLicences(MavenPomLicenseSpec licences) {
        licences.license((licence) -> {
            licence.getName().set("MIT License");
            licence.getUrl().set("https://gitee.com/quan100/quan/blob/main/LICENSE");
        });
    }

    private void customizeDevelopers(MavenPomDeveloperSpec developers) {
        developers.developer((developer) -> {
            developer.getName().set("JavaQuan");
            developer.getEmail().set("javaquan@qq.com");
            developer.getOrganization().set("JavaQuan");
            developer.getOrganizationUrl().set("https://www.javaquan.cn");
        });
    }

    private void customizeScm(MavenPomScm scm, Project project) {
        if (!isUserInherited(project)) {
            scm.getConnection().set("scm:git:https://gitee.com/quan100/quan.git");
            scm.getDeveloperConnection().set("scm:git@gitee.com:quan100/quan.git");
        }
        scm.getUrl().set("https://gitee.com/quan100/quan");
    }

    private void customizeIssueManagement(MavenPomIssueManagement issueManagement) {
        issueManagement.getSystem().set("Gitee");
        issueManagement.getUrl().set("https://gitee.com/quan100/quan/issues");
    }

    private boolean isUserInherited(Project project) {
        return "quan-parent".equals(project.getName()) || "quan-dependencies".equals(project.getName());
    }
