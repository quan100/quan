...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  LAYIM.prototype.config = function(options){
    ...
    layui.each(Array(5), function(index){
      skin.push(layui.cache.layimAssetsPath + 'skin/'+ (index+1) +'.jpg')
    });
    options = options || {};
  ...
  var elemAddTpl = ['<div class="layim-add-box">'
    ,'<div class="layim-add-img"><img class="layui-circle" src="{{ d.data.avatar || (layui.cache.layimAssetsPath + \'images/default.png\') }}"><p>{{ d.data.name||"" }}</p></div>'
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar || (layui.cache.layimAssetsPath + \'images/default.png\') }}"><cite>'
    ,'{{# if(d.mine){ }}'
  ...
  var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar || (layui.cache.layimAssetsPath + \'images/default.png\') }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var post = function(options, callback, tips){
    ...
    return $.ajax({
      ...
      ,success: function(res){
        var successCode = options.result.code || 0;
        res.code == successCode
          ? callback && callback(res.data||{})
        : options.fail?options.fail(res) : layer.msg(res.msg || res[options.result.msgKey] || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'), {
          time: 5000
        });
      },error: function(err, msg){
        options.error?options.error(err, msg) : window.console && console.log && console.error('LAYIM_DATE_ERROR：' + msg);
      }
    });
  };
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar || (layui.cache.layimAssetsPath + 'images/default.png')) +'"><span>'+ (content||cache.base.title||'我的 IM') +'</span>'
  ...
  var setChatMin = function(newMsg){
    ...
    setChatMin.index = layer.open({
      ...
      ,resize: false
      ,area: ['182px', '50px']
      ,content: '<img id="layui-layim-min" src="'+ (thatChat.avatar || (layui.cache.layimAssetsPath + 'images/default.png')) +'"><span>'+ thatChat.name +'</span>'
      ,success: function(layero, index){
    ...
    return popAdd.index = layer.open({
      ...
      ,content: laytpl(elemAddTpl).render({
        data: {
          ...
          ,avatar: data.avatar || layui.cache.layimAssetsPath + 'images/default.png'
    ...
    layero.css({
      'background-image': skin ? 'url('+ skin +')' : function(){
        return cache.base.initSkin 
          ? 'url('+ (layui.cache.layimAssetsPath + 'skin/'+ cache.base.initSkin) +')'
        : 'none';
      }()
    });
  };
  ...
  var sendMessage = function(){
    var data = {
      ...
      ,avatar: cache.mine ? cache.mine.avatar : (layui.cache.layimAssetsPath + 'images/default.png')
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      ...
      var param = {
        ...
        ,avatar: param.mine.avatar || layui.cache.layimAssetsPath + 'images/default.png'
        ,id: param.to.id
        ,type: param.to.type
    ...
    if (window.Notification){
      if(Notification.permission === 'granted'){
        var notification = new Notification(data.title||'', {
          body: data.content || ''
          ,icon: data.avatar || (layui.cache.layimAssetsPath + 'images/default.png')
        });
      }else {
  ...
  var voice = function() {
    ...
    audio.src = layui.cache.layimAssetsPath + 'voice/'+ cache.base.voice;
    audio.play();
  };
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if((!layimChat && data.content) || index === -1){
      ...
      if(data.type === 'group'){
        layui.each(cache.group, function(index, item){
          if(item.id == data.id){
            group.avatar = item.avatar || layui.cache.layimAssetsPath + 'images/default.png';
            return true;
          }
        });
      }
      if(!data.system){
        if(cache.base.notice){
          notice({
            ...
            ,avatar: group.avatar || data.avatar || layui.cache.layimAssetsPath + 'images/default.png'
          });
        }
        return setChatMin({
          name: '收到新消息'
  ...
  var faces = function(){
    ...
    layui.each(alt, function(index, item){
      arr[item] = layui.cache.layimAssetsPath + 'images/face/'+ index + '.gif';
    });
    return arr;
  }();
  ...
  var anim = 'layui-anim-upbit', events = {
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ (data[l].avatar || layui.cache.layimAssetsPath + 'images/default.png') +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
    ...
    ,setSkin: function(othis){
      ...
      layui.each(call.setSkin, function(index, item){
        var filename = (src||'').replace(layui.cache.layimAssetsPath +'skin/', '');
          ...
          ,success: function(layero){
            ...
            post(members, function(res){
              layui.each(res.list, function(index, item){
                li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="'+ (item.avatar || layui.cache.layimAssetsPath + 'images/default.png') +'"><cite>'+ item.username +'</cite></a></li>';
              ...
              ul.find('li').on('click', function(){
                ...
                popchat({
                  name: info.username
                  ,type: 'friend'
                  ,avatar: info.avatar || layui.cache.layimAssetsPath + 'images/default.png'
                  ,id: info.id
                });
                hide();
              });
...
}).link(layui.cache.layimAssetsPath + 'layim.css', 'skinlayimcss') //加载 css;
