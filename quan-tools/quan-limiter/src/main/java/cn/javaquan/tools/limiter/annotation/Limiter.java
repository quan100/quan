...
import cn.javaquan.tools.limiter.executor.DefaultLimiterExecutor;
...
public @interface Limiter {
     ...
     * 默认由方法的全限定名 + 方法名组成
     * <p>
     * 令牌名称是令牌的组成部分，令牌的组成由 {@link LimiterProperties#tokenPrefix} + {@code name} +
     * {@link #params} + {@link #arrayParams} 构成。
     * @return 令牌前缀
     */
    String name() default "";
     ...
     * @return 执行器的具体实现类
     */
    Class<? extends LimiterExecutor<?>> executor() default DefaultLimiterExecutor.class;

    /**
     * 令牌参数的表达式.
     * <p>
     * 通过令牌参数模版，从方法的请求参数中解析具体的参数值，并拼接到令牌后面。 当配置多个参数时，按照顺序拼接。
     * <p>
     ...
     * <p>
     * true: 令牌参数可以为空，即任何情况下，均会创建令牌。 false:
     * 令牌参数不可以为空，要求令牌参数必须存在。如果令牌参数不存在，则不会创建令牌；意味着限制器将不生效。
     * @return 令牌参数是否可以为空
     */
    boolean nullable() default true;

    /**
     * 令牌的有效期. 单位：毫秒 默认值：{@link LimiterProperties#leaseTime}
     * @return 令牌的有效期
     */
    long leaseTime() default 0L;

    /**
     * 获取令牌的最大等待时间. 单位：毫秒 默认值：{@link LimiterProperties#waitTime}
     * @return 获取令牌的最大等待时间
     */
     ...
     * 令牌数组参数的阀值.
     * <p>
     * 只有当配置 {@link #arrayParams} 参数时，该配置生效。默认值为 {@code -1}，表示不限制。 当根据
     * {@link #arrayParams} 模版解析的数组元素数量超过阀值时，超出的参数不会生成 token 。
     * @return 令牌数组参数的阀值
     */
    int arrayParamsThreshold() default -1;

    /**
     * 当请求被限制器拦截，默认抛出 {@link LimiterException} 异常. 通过该参数可以定义异常信息.支持
     * {@link String#format(String, Object...)} 格式化信息内容。可选的格式化参数为 {@link #params}
     * 模版解析的参数值。
     * @return 默认的异常信息
     */
     ...
     * 程序执行完成后，是否自动释放令牌.
     * <p>
     * 自动释放的场景： 1. 当程序执行完成后，自动释放。 2. 当令牌有效期超过 {@link #leaseTime} 后，自动释放。
     * <p>
